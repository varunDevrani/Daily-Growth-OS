...
def logout(
    ...
    return controllers.logout(payload, db)


@router.post(
    "/refresh", status_code=HTTPStatus.OK, response_model=SuccessResponse[TokenResponse]
)
def refresh(
    payload: RefreshTokenRequest, db: Session = Depends(get_db)
) -> SuccessResponse[TokenResponse]:
    return controllers.refresh(payload, db)
