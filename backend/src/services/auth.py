...
def signup(
    ...
    user_data = User(email=payload.email, password_hash=hash_password(payload.password))
    db.add(user_data)
    db.flush()
...
def login(payload: LoginRequest, db: Session) -> TokenResponse:
    ...
    if user_data is None or not verify_password(
        payload.password, user_data.password_hash
    ):
        raise DomainException(
...
def logout(payload: RefreshTokenRequest, db: Session) -> None:
    _ = consume_refresh_token(payload, db)


def refresh(payload: RefreshTokenRequest, db: Session) -> TokenResponse:
