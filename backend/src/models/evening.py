...
class Evening(IDMixin, TimestampMixin, Base):
    ...
    user_id: Mapped[datetime.date] = mapped_column(ForeignKey("users.id"))
    ...
    distraction: Mapped[str] = mapped_column(String, nullable=False)  
    mood_rating: Mapped[int] = mapped_column(Integer, nullable=False)  
    lesson: Mapped[str] = mapped_column(String, nullable=False)
    energy_rating: Mapped[int] = mapped_column(Integer, nullable=False)  
